...
export function CheckoutContent() {
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const reference = `AM-${Date.now().toString().slice(-8)}`;
    window.localStorage.setItem("automart-last-order", JSON.stringify({ reference, total: cartTotal, createdAt: new Date().toISOString() }));
    clearCart();
    setComplete(true);
  };
  if (complete) return <section className="section commerce-page"><div className="container"><div className="form-success"><CheckCircle2/><h2>Order received</h2><p>Your frontend demo order has been saved on this device. No payment was processed.</p><Link className="primary-action" href="/shop">Back to Products</Link></div></div></section>;
  return (
    <section className="section commerce-page">
